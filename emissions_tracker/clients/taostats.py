...
class TaoStatsAPIClient(WalletClientInterface, PriceClient):
    ...
    def __init__(self):
        ...
        if not self.config.api_key:
            raise ValueError(
                "TAOSTATS_API_KEY is required. Sign up at https://dash.taostats.io/"
            )
    ...
    def _fetch_with_pagination(
        self, url: str, params: dict, per_page: int = 200, context: str = ""
    ) -> List[Dict[str, Any]]:
        ...
        while True:
            if (
                ...
                and time.time() - self._last_call_time < self._rate_limit_seconds
            ):
                sleep_time = self._rate_limit_seconds - (
                    time.time() - self._last_call_time
                )
    ...
    def get_transfers(
        ...
        try:
            ...
            transfer_data = self._fetch_with_pagination(
                url, params, per_page=500, context="transfers"
            )
            ...
            for t in transfer_data:
                transfer = TaoStatsTransfer(
                    ...
                    from_address=TaoStatsAddress(
                        ss58=t["from"]["ss58"], hex=t["from"]["hex"]
                    ),
                    to_address=TaoStatsAddress(
                        ss58=t["to"]["ss58"], hex=t["to"]["hex"]
                    ),
                )
    ...
    def get_delegations(
        ...
        try:
            ...
            delegation_data = self._fetch_with_pagination(
                url, params, per_page=500, context="delegations"
            )
            ...
            for d in delegation_data:
                delegation = TaoStatsDelegation(
                    ...
                    nominator=TaoStatsAddress(
                        ss58=d["nominator"]["ss58"], hex=d["nominator"]["hex"]
                    ),
                    delegate=TaoStatsAddress(
                        ss58=d["delegate"]["ss58"], hex=d["delegate"]["hex"]
                    ),
                    netuid=int(d["netuid"]),
                    amount=int(d["amount"]),
                    alpha=int(d["alpha"]),
                    ...
                    transfer_address=(
                        TaoStatsAddress(
                            ss58=d["transfer_address"]["ss58"],
                            hex=d["transfer_address"]["hex"],
                        )
                        if d.get("transfer_address")
                        else None
                    ),
                    fee=d.get("fee"),
                )
    ...
    def get_stake_balance_history(
        ...
        try:
            ...
            history_data = self._fetch_with_pagination(
                url, params, per_page=500, context="stake_balance_history"
            )
            ...
            for h in history_data:
                balance = TaoStatsStakeBalance(
                    ...
                    hotkey=TaoStatsAddress(
                        ss58=h["hotkey"]["ss58"], hex=h["hotkey"]["hex"]
                    ),
                    coldkey=TaoStatsAddress(
                        ss58=h["coldkey"]["ss58"], hex=h["coldkey"]["hex"]
                    ),
                    netuid=h["netuid"],
                    balance=h["balance"],
    ...
    def get_account_history(
        ...
        history_data = self._fetch_with_pagination(
            url, params, per_page=200, context="account_history"
        )
        ...
        for h in history_data:
            history = TaoStatsAccountHistory(
                address=TaoStatsAddress(
                    ss58=h["address"]["ss58"], hex=h["address"]["hex"]
                ),
                network=h["network"],
                block_number=h["block_number"],
                timestamp=h["timestamp"],
        ...
        try:
            if symbol != "TAO":
                raise PriceNotAvailableError(
                    f"Taostats API only supports TAO, got {symbol}"
                )
            ...
            data = self._fetch_with_pagination(
                url, params, context="price_at_timestamp"
            )
            ...
            if data and len(data) > 0:
                ...
                price_time = datetime.fromisoformat(
                    closest_price["created_at"].replace("Z", "+00:00")
                ).strftime("%Y-%m-%d %H:%M:%S")
                print(
                    f"✓ Got {symbol} price from Taostats: ${price:.2f} at {price_time}"
                )
                self._price_bucket_cache[bucket] = price
                return price

            raise PriceNotAvailableError(
                f"No price data available for {symbol} within ±30 minutes of {datetime.fromtimestamp(timestamp, tz=timezone.utc)}"
            )
    ...
    def get_prices_in_range(self, symbol: str, start_time: int, end_time: int):
        ...
        data = self._fetch_with_pagination(
            url, params, per_page=500, context="price_range"
        )
        ...
        for item in data:
            ...
            prices.append({"timestamp": ts, "price": float(item["price"])})
        ...
        prices = sorted(
            [{"timestamp": ts, "price": unique[ts]} for ts in unique],
    ...
    def get_current_price(self, symbol: str) -> float:
        ...
        try:
            ...
            if data and len(data) > 0:
                ...
                print(
                    f"✓ Got current {symbol} price from Taostats: ${price:.2f} at {price_time}"
                )
                return price
