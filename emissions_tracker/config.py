...
class TrackerSettings(BaseSettings):
    ...
    # Wallet addresses
    brokerage_ss58: str = Field(
        ..., alias="BROKER_SS58", description="Kraken deposit address"
    )
    validator_ss58: str = Field(
        ..., alias="VALIDATOR_SS58", description="Validator hotkey"
    )
    payout_coldkey_ss58: str = Field(
        ...,
        alias="PAYOUT_COLDKEY_SS58",
        description="Coldkey wallet that receives smart contract payouts",
    )
    smart_contract_ss58: str = Field(
        ...,
        alias="SMART_CONTRACT_SS58",
        description="Smart contract address for filtering contract income",
    )

    # Mining tracker addresses (optional)
    miner_hotkey_ss58: str = Field(
        None,
        alias="MINER_HOTKEY_SS58",
        description="Miner hotkey for mining emissions tracking",
    )
    miner_coldkey_ss58: str = Field(
        None,
        alias="MINER_COLDKEY_SS58",
        description="Miner coldkey for mining emissions tracking (if different from payout_coldkey_ss58)",
    )

    # Payment tracker addresses (optional)
    payment_coldkey_ss58: str = Field(
        None,
        alias="PAYMENT_COLDKEY_SS58",
        description="Coldkey wallet used solely for receiving third-party payments",
    )

    # Google Sheets
    tracker_sheet_id: str = Field(
        ...,
        alias="TRACKER_SHEET_ID",
        description="Google Sheet ID for smart contract tracking",
    )
    mining_tracker_sheet_id: str = Field(
        None,
        alias="MINING_TRACKER_SHEET_ID",
        description="Google Sheet ID for mining emissions tracking",
    )
    payment_tracker_sheet_id: str = Field(
        None,
        alias="PAYMENT_TRACKER_SHEET_ID",
        description="Google Sheet ID for payment tracking",
    )
    tracker_google_credentials: str = Field(
        ...,
        alias="TRACKER_GOOGLE_CREDENTIALS",
    ...
    lot_strategy: CostBasisMethod = Field(
        CostBasisMethod.HIFO,
        alias="LOT_STRATEGY",
        description="Lot consumption strategy: CostBasisMethod.HIFO or CostBasisMethod.FIFO",
    )
...
class CoinMarketCapSettings(BaseSettings):
    ...
    cmc_api_key: Optional[str] = Field(
        None, alias="COINMARKETCAP_API_KEY", description="CoinMarketCap API key"
    )
