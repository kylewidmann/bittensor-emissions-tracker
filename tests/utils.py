...
def compute_staking_emissions_from_balances(
    ...
    get_lot_id,
    source_type: SourceType = SourceType.STAKING,
) -> List[AlphaLot]:
    """Compute emission lots from balance history and delegation events.

    Works for both staking and mining emissions since both derive income from
    daily balance deltas.  Pass ``SourceType.MINING`` for mining trackers.
    ...
    Args:
        ...
        source_type: SourceType to assign to created lots
    ...
    daily_balances = filter_balances_by_date_range(
        daily_stake_balances, extended_start, end_ts
    )
    ...
    for i in range(1, len(daily_balances)):
        ...
        balance_change_alpha_rao = (
            curr_day.balance_as_alpha_rao - prev_day.balance_as_alpha_rao
        )

        alpha_price_tao_rao = (
            curr_day.balance_as_tao_rao / curr_day.balance_as_alpha_rao
        )
...
def filter_balances_by_date_range(
    daily_stake_balances: Dict[str, TaoStatsStakeBalance], start_ts: int, end_ts: int
    ...
    balances = [
        b
        for b in daily_stake_balances.values()
...
def filter_delegation_events_by_date_range(
    event_data: List[TaoStatsDelegation], start_ts: int, end_ts: int
    ...
    """
    events = [e for e in event_data if start_ts <= e.timestamp_unix <= end_ts]
...
def _consume_alpha_lots(
    ...
    available_lots = [
        ...
        if lot.timestamp <= timestamp and lot.alpha_rao_remaining > 0
    ]
    ...
    if cost_basis_method == CostBasisMethod.FIFO:
        ...
        raise ValueError(
            f"Invalid cost_basis_method: {cost_basis_method}. Must be CostBasisMethod.FIFO or CostBasisMethod.HIFO"
        )
...
def consume_alpha_lots_for_expense(
    ...
    consumed_lots, cost_basis = _consume_alpha_lots(
        alpha_lots, delegation.alpha, delegation.timestamp_unix, cost_basis_method
    )
    ...
    holding_period_days = (delegation.timestamp_unix - newest_lot_timestamp) / (
        24 * 60 * 60
    )
    gain_type = (
    ...
    expense = Expense(
        ...
        transfer_address=(
            delegation.transfer_address.ss58 if delegation.transfer_address else ""
        ),
...
def consume_alpha_lots_for_sale(
    ...
    tao_lot = TaoLot(
        ...
        usd_basis=alpha_sale.usd_proceeds - alpha_sale.network_fee_usd,
...
def consume_tao_lots(
    ...
    available_lots: list[TaoLot] = [
        lot
        for lot in tao_lots
        if lot.timestamp <= taostats_transfer.timestamp_unix and lot.rao_remaining > 0
    ]
    ...
    newest_lot_timestamp = min(c.acquisition_timestamp for c in consumed_lots)
    holding_period_days = (taostats_transfer.timestamp_unix - newest_lot_timestamp) / (
        24 * 60 * 60
    )
    gain_type = (
        GainType.LONG_TERM if holding_period_days >= 365 else GainType.SHORT_TERM
    ...
    fee_cost_basis = (
        (total_cost_basis * (taostats_transfer.fee_rao / total_outflow_rao))
