...
from tests.utils import (
    consume_alpha_lots_for_expense,
    consume_alpha_lots_for_sale,
    consume_tao_lots,
    filter_delegation_events_by_date_range,
)
...
@pytest.fixture
def daily_stake_events(
    stake_events: list[TaoStatsDelegation],
) -> Dict[str, list[TaoStatsDelegation]]:
...
def get_opening_tao_lot(
    ...
    def _get_opening_tao_lot(date: datetime):
        # Get balance from previous day
        target_date_str = (date - timedelta(days=1)).strftime("%Y-%m-%d")
        account_history = next(
            ah for ah in account_histories if ah.day == target_date_str
        )
        ...
        return TaoLot(
            ...
            timestamp=int(date.timestamp()) - 1,
            block_number=0,
            rao=tao_balance_rao,
...
def get_opening_alpha_lot(
    ...
    def _get_opening_alpha_lot(date: datetime):
        # Get balance from previous day
        target_date_str = (date - timedelta(days=1)).strftime("%Y-%m-%d")
        balance = next(
            b for b in daily_stake_balances.values() if b.day == target_date_str
        )
        tao_price = historical_prices.get_price_for_date(target_date_str)
        usd_fmv = balance.balance_as_tao_float * tao_price
        usd_per_alpha = (
            (balance.balance_as_tao_float * tao_price) / balance.balance_as_alpha_float
            if balance.balance_as_alpha_rao > 0
            else 0
        )
...
def compute_expected_contract_income_lots(
    ...
    def _compute_expected_contract_income_lots(
        ...
        filtered_events = filter_delegation_events_by_date_range(
            stake_events, start_ts, end_ts
        )
        transfer_income_events = [
            event
            ...
            if (
                ...
                and event.transfer_address
            )
        ]
        lots = []
        for e in transfer_income_events:
            is_contract = e.transfer_address.ss58 == contract_address
            ...
            lots.append(
                AlphaLot(
                    ...
                    usd_per_alpha=(
                        float(e.usd) / (int(e.alpha) / 1e9) if int(e.alpha) > 0 else 0
                    ),
                    usd_fmv=float(e.usd),
                    tao_equivalent=e.tao,
                    extrinsic_id=e.extrinsic_id,
                    transfer_address=(
                        e.transfer_address.ss58 if e.transfer_address else ""
                    ),
                    status=LotStatus.OPEN,
                    source_type=source,
                )
...
def compute_expected_expenses(
    ...
    def _compute_expected_expenses(
        ...
        for e in stake_events:
            if (
                e.action == "UNDELEGATE"
                and e.is_transfer == True
                and e.transfer_address is not None
                and e.transfer_address.ss58 != TEST_SMART_CONTRACT_SS58
        ...
        for event in expense_undelegates:
            expense = consume_alpha_lots_for_expense(
                get_expense_id(), alpha_lots, event, cost_basis_method
            )
...
def compute_expected_deposit_lots(
    ...
    def _compute_expected_deposit_lots(
        ...
        for deposit in deposits:
            ...
            timestamp = (
                (deposit.timestamp_unix + SECONDS_PER_DAY // 2) // SECONDS_PER_DAY
            ) * SECONDS_PER_DAY
            ...
            deposit_lots.append(
                TaoLot(
                    ...
                    status=LotStatus.OPEN,
                    notes=(
                        f"Deposit from {deposit.from_address.ss58[:8]}..."
...
def compute_expected_sales(
    ...
    [datetime, datetime, Optional[CostBasisMethod]],
    ...
    def _compute_expected_sales(
        ...
        with id_context():
            ...
            alpha_start_date = (
                opening_lot_date if opening_lot_date is not None else start_date
            )
            ...
            for sale in sales:
                ...
                associated_transfers = [
                    t for t in transfer_events if t.extrinsic_id == sale.extrinsic_id
                ]
...
def compute_expected_transfers(
    transfer_events: list[TaoStatsTransfer],
    compute_expected_sales: Callable[
        [datetime, datetime, Optional[CostBasisMethod]],
        tuple[list[AlphaSale], list[TaoLot]],
    ],
    ...
    [datetime, datetime, str, str, Optional[CostBasisMethod], Optional[datetime]],
    tuple[list[AlphaSale], list[TaoLot]],
]:
    """
    ...
    def _compute_expected_transfers(
        ...
        with id_context():
            ...
            if cost_basis_method == CostBasisMethod.FIFO:
                sorted_tao_lots: list[TaoLot] = sorted(
                    tao_lots, key=lambda x: x.timestamp
                )
            else:  # HIFO
                sorted_tao_lots: list[TaoLot] = sorted(
                    tao_lots, key=lambda x: -x.usd_per_tao
                )
            ...
            for taostats_transfer in brokerage_transfers:
                ...
                timestamp = (
                    (taostats_transfer.timestamp_unix + SECONDS_PER_DAY // 2)
                    // SECONDS_PER_DAY
                ) * SECONDS_PER_DAY
                tao_price = historical_prices.get_price_for_timestamp(timestamp)
                transfer = consume_tao_lots(
                    get_transfer_id(), sorted_tao_lots, taostats_transfer, tao_price
                )
