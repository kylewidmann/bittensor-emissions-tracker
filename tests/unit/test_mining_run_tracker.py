...
def test_run_mining_tracker(
    ...
    from emissions_tracker.models import AlphaLot, SourceType as ST, LotStatus
    all_lots = []
    ...
    opening_lots = [lot for lot in all_lots if lot.source_type.value == 'Opening Balance']
    emission_lots = [lot for lot in all_lots if lot.source_type.value == 'Staking']
    ...
    print(f"Emission lots: {actual_emission_count} (expected: {expected_emission_count})")
    
    # Verify opening lots were created (1 ALPHA opening lot)
    assert len(opening_lots) == 1, f"Expected 1 opening ALPHA lot, got {len(opening_lots)}"
    assert opening_lots[0].lot_id == 'ALPHA-0001', "Opening lot should be ALPHA-0001"
    
    # Verify emission lot count matches expected
    assert actual_emission_count == expected_emission_count, \
    ...
    for i, (expected, actual) in enumerate(zip(expected_sorted, actual_sorted)):
        ...
        expected_date = datetime.fromtimestamp(expected.timestamp, tz=timezone.utc).date()
        ...
        assert actual.alpha_rao == expected.alpha_rao, \
        ...
        # Verify source type
        assert actual.source_type.value == 'Staking', \
            f"Lot {i+1} should have STAKING source type"
        
        # Verify lot status is Open
        assert actual.status.value == 'Open', \
            f"Lot {i+1} should have Open status"
