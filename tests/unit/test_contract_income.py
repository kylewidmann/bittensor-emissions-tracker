...
def test_process_contract_income(
    contract_tracker, compute_expected_contract_income_lots, start_date, end_date
):
    ...
    with patch.object(
        contract_tracker, "_resolve_time_window", return_value=(start_time, end_time)
    ):
    ...
    ), f"Expected {expected_count} lots, got {actual_count}"
    ...
    for i, (expected, actual) in enumerate(zip(expected_sorted, actual_sorted)):
        ...
        assert (
            actual.alpha_rao == expected_alpha_rao
        ...
        assert (
            actual.usd_fmv == expected_usd_fmv
        ), f"Lot {i+1} USD FMV mismatch: {actual.usd_fmv} != {expected_usd_fmv}"

        # Verify source type matches expected
        assert (
            actual.source_type == expected.source_type
        ), f"Lot {i+1} source type mismatch: {actual.source_type} != {expected.source_type}"

        # Verify lot ID format
        assert actual.lot_id.startswith(
            "ALPHA-"
