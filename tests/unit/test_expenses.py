...
def test_process_expenses(
    ...
    seeded_alpha_lots = seed_contract_sheets(seed_date, start_date)
    ...
    expected_expenses: list[Expense] = compute_expected_expenses(
    ...
    for i, (actual, expected) in enumerate(zip(actual_expenses, expected_expenses)):
        ...
        assert actual.gain_type == expected.gain_type, \
            f"Expense {i+1}: gain_type mismatch - actual: {actual.gain_type}, expected: {expected.gain_type}"
