...
def verify_stake_balance_matches_raw(
    stake_balance: TaoStatsStakeBalance, raw: Dict[str, Any]
):
...
def verify_delegation_matches_raw(delegation: TaoStatsDelegation, raw: Dict[str, Any]):
    ...
    assert (
        delegation.usd == raw["usd"]
        if isinstance(raw["usd"], float)
        else float(raw["usd"])
    )
...
def test_get_stake_balance_history_returns_models_matching_raw_json(client):
    ...
    with patch("requests.get") as mock_get:
        ...
        # Call the actual client method
        stake_balances = client.get_stake_balance_history(
            64, "hotkey", "coldkey", 0, 9999999999
        )
