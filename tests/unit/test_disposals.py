...
from emissions_tracker.models import AlphaSale, Expense, TaoTransfer, CostBasisMethod
...
def test_process_disposals(
    ...
    # Verify Sales
    # ==========================================================================
    assert len(actual_sales) == len(expected_sales), \
    ...
    for i, (actual, expected) in enumerate(zip(actual_sales_sorted, expected_sales_sorted)):
        assert actual.timestamp == expected.timestamp, \
            f"Sale {i+1}: timestamp mismatch: {actual.timestamp} != {expected.timestamp}"
        assert actual.alpha_disposed == expected.alpha_disposed, \
        ...
        assert actual.tao_slippage == expected.tao_slippage, \
            f"Sale {i+1}: TAO slippage mismatch: {actual.tao_slippage} != {expected.tao_slippage}"
        assert actual.network_fee_tao == expected.network_fee_tao, \
        ...
        assert len(actual.consumed_lots) > 0, \
            f"Sale {i+1}: should have consumed lots"
    ...
    for i, (actual, expected) in enumerate(zip(actual_expenses_sorted, expected_expenses_sorted)):
        assert actual.timestamp == expected.timestamp, \
    ...
    for i, (actual, expected) in enumerate(zip(actual_transfers_sorted, expected_transfers_sorted)):
        assert actual.timestamp == expected.timestamp, \
        ...
        assert len(actual.consumed_tao_lots) > 0, \
