...
def test_run_tracker(
    ...
    print(f"\n=== Verification ===")
    print(f"Expected ALPHA: {expected_alpha_rao / 1e9:.9f} ({expected_alpha_rao:,} RAO)")
    ...
    assert alpha_diff <= alpha_tolerance, \
        f"ALPHA balance mismatch: {int(final_alpha_rao):,} vs expected {expected_alpha_rao:,} (diff: {alpha_diff:,} RAO)"
    print(f"✓ ALPHA balance: {int(final_alpha_rao):,} RAO (expected {expected_alpha_rao:,}, diff {alpha_diff:.0f})")
    ...
    assert tao_diff <= tao_tolerance, \
        f"TAO balance mismatch: {final_tao_rao:,} vs expected {expected_tao_rao:,} (diff: {tao_diff:,} RAO)"
    print(f"✓ TAO balance: {final_tao_rao:,} RAO (expected {expected_tao_rao:,}, diff {tao_diff})")
